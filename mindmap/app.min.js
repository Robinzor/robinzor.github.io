...
function render(){
  ...
  state.nodes.forEach(n=>{
    ...
    [editBg, editText].forEach(el=>{
      // prevent pointerdown from bubbling (avoid triggering selection/drag)
      el.addEventListener('pointerdown', ev=>{ ev.stopPropagation(); ev.preventDefault(); });
      // Only open editor on click (mouse) to avoid opening on mousedown/drag start
    ...
    // If the inline editor is open, hide the plus/minus buttons globally to avoid accidental changes
    // Otherwise let them inherit the parent `controls` visibility (so they only show on hover/selection)
    if(editingNode){
      // hide plus/minus and edit while editing
      plusBg.setAttribute('visibility','hidden');
      plusText.setAttribute('visibility','hidden');
      minusBg.setAttribute('visibility','hidden');
      minusText.setAttribute('visibility','hidden');
      editBg.setAttribute('visibility','hidden');
      editText.setAttribute('visibility','hidden');
    } else {
      // let buttons inherit visibility from the parent controls group
      plusBg.removeAttribute('visibility');
      plusText.removeAttribute('visibility');
      minusBg.removeAttribute('visibility');
      minusText.removeAttribute('visibility');
      editBg.removeAttribute('visibility');
      editText.removeAttribute('visibility');
    }
    ...
    // Hover handlers removed: controls now only appear when selected or persistent.
    // Previously pointerenter/pointerleave showed/hid controls on hover; that behavior was removed per user request.
    ...
    group.addEventListener('click', (ev)=>{
      ...
      selectedNodeId = n.id;
      // Ensure only one node's controls are persistent at a time
    ...
    group.addEventListener('pointerdown', (ev)=>{
      ...
      // On touch, make this node the sole persistent control so it stays visible after a tap
      if(ev.pointerType === 'touch' || ev.pointerType === 'mouse' || ev.pointerType === 'pen'){
        try{
          // Select immediately on pointerdown for touch and desktop pointer types
          persistentControls.clear();
          persistentControls.add(n.id);
          selectedNodeId = n.id;
          // Re-render so any previously persistent controls are cleared visually
          render();
        }catch(e){}
      }
    ...
    // Double-click editing removed so editing is only via the pencil control (works for desktop and mobile)
...
svg.addEventListener('pointermove', (ev)=>{
  ...
  if(dragging){
    ...
    // mark that the pointer moved while dragging so we can distinguish a real drag from a click
    draggingMoved = true;
    render();
...
svg.addEventListener('pointerup', (ev)=>{
  ...
  if(dragging){
    ...
    const wasMoved = !!draggingMoved;
    dragging = null;
    draggingMoved = false;
    justDragged = wasMoved;
    if(justDragged){ setTimeout(()=>{ justDragged = false; }, 200); }
    scheduleAutosave();
  }
  panActive = false;
  lastPointer = null;
...
function showEditor(node){
  ...
  // Re-render so controls (like plus/minus) update visibility while editing
  // Clear any persistent controls so they disappear while editing
  persistentControls.clear();
  selectedNodeId = null;
  render();
}
...
function hideEditor(save){
  ...
  // Ensure UI updates after editor closes so controls (plus/minus) are restored/hidden correctly
  render();
}
...
// When the user types in the inline editor, hide any visible controls immediately
...
// If the user clicks/taps anywhere outside the inline editor, save & close it.
// Use capture phase so we catch the interaction before other handlers may stop propagation.
document.addEventListener('pointerdown', (ev)=>{
  try{
    if(!editingNode) return;
    if(!editor) return;
    if(editor.contains(ev.target)) return;
    // click is outside the editor — save changes
    hideEditor(true);
  }catch(e){}
}, true);
...
if(!loadMapFromLocalStorage()){
  ...
  // Do not seed default nodes — user starts from blank slate.

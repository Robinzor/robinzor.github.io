  ...
  state.nodes.forEach(n=>{
    ...
    if(state.nodeShape === 'box'){
      ...
      lines.forEach((ln, idx)=>{
        ...
        t.setAttribute('x', '0');
        const dy = (idx === 0) ? -( (lines.length-1) * lineHeight )/2 + (lineHeight/2) : lineHeight;
    ...
    controls.setAttribute('visibility', selectedNodeId === n.id ? 'visible' : 'hidden');
    ...
    plusBg.addEventListener('click', (ev)=>{ ev.stopPropagation(); addChild(n); });
    plusText.addEventListener('click', (ev)=>{ ev.stopPropagation(); addChild(n); });
    minusBg.addEventListener('click', (ev)=>{ ev.stopPropagation(); removeNode(n.id); });
    minusText.addEventListener('click', (ev)=>{ ev.stopPropagation(); removeNode(n.id); });
    ...
    const scheduleHide = ()=>{ if(hideTimeout) clearTimeout(hideTimeout); hideTimeout = setTimeout(()=>{ controls.setAttribute('visibility','hidden'); hideTimeout = null; }, 1000); };

    group.addEventListener('pointerenter', ()=>{ showControls(); });
...
if(!loadMapFromLocalStorage()){
  const a=addNode(300,200,'Root');
  const b=addNode(500,150,'Child A');
  const c=addNode(500,250,'Child B');
